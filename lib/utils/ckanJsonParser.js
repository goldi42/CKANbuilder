...
class CkanJsonParser {
    ...
    parseCkanJson () {
        ...
        if(!fs.existsSync(fullFilePath)) {
            ...
            if(!fs.existsSync(fullFilePath)) {
                this._getLogger().info('Legacy file not found, too! Please create a ckan.json File!');
            } else {
                this._getLogger().info('Legacy file found. Please rename this file to ckan.json!');
            }
        }
        try {
            this._ckanJson = require(fullFilePath);
        } catch (error) {
            this._getLogger().info('Error by Parsing ckan json, see error Message: \n ' + error);
        }
    }
