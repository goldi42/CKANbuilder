...
class CkanJsonParser {
    ...
    parseCkanJson () {
        ...
        if(!fs.existsSync(fullFilePath)) {
            ...
            if(!fs.existsSync(fullFilePath)) {
                LogService.getInstance().warn('Legacy file not found, too! Please create a ckan.json File!');
            } else {
                LogService.getInstance().warn('Legacy file found. Please rename this file to ckan.json!');
            }
        }
        try {
            this._ckanJson = require(fullFilePath);
        } catch (error) {
            LogService.getInstance().error('Error by Parsing ckan json, see error Message: \n ' + error);
        }
    }
}
