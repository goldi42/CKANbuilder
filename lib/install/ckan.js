...
class CkanInstaller {
     ...
     * @param {string} pipPEP517 - use pip option for installing CKAN
     */
    static install (ckanVersion, destinationPath, pipPEP517) {
        let logger = LogService.getInstance();
        logger.info('Start to install CKAN in Version: ' + ckanVersion);
        fs.ensureDirSync(destinationPath);
        let ckanInstallResults = shell.exec(`pip install ${pipPEP517} -e git+https://github.com/ckan/ckan.git@ckan-${ckanVersion}#egg=ckan --src ${destinationPath}`, {silent:true});
