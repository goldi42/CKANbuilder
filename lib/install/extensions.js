...
class ExtensionInstaller {
     ...
     * @param {string} pipPEP517 - use pip option for installing CKAN extensions
     */
    static install (extensions, installDirectory, pipPEP517) {
        let logger = LogService.getInstance();
        extensions.forEach(extension => {
            let pipCommand = '';
            switch (extension.type.toLowerCase()) {
            case 'local':
                pipCommand = `pip install ${pipPEP517} -e ${path.join(installDirectory, extension.name)}`;
                break;
            case 'vcs':
                pipCommand = `pip install ${pipPEP517} -e ${extension.repository}@${extension.version}#egg=${extension.name} --src ${installDirectory}`;
                break;
            default:
                pipCommand = `pip install ${pipPEP517} ${extension.name}~=${extension.version}`;
                break;
            }
            ...
            if (extension.ignore_requirement_file) {
                logger.info('Ignore requirement file');
            } else {
                ...
                if (dependencyFile !== null && typeof (dependencyFile) !== 'undefined') {
                    ...
                    let extensionRequirementsResult = shell.exec(`pip install ${pipPEP517} -r ${path.join(installDirectory, extensionDirectory, dependencyFile)}`, {silent: true});
